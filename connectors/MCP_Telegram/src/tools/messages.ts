...
export const messageTools: Tool[] = [
  ...
  {
    name: "sendLivePhoto",
    description: "Send a live photo (video that plays like a photo). On success, the sent Message is returned.",
    inputSchema: {
      type: "object",
      properties: {
        chat_id: { type: ["string", "integer"], description: "Unique identifier for the target chat or username" },
        message_thread_id: { type: "integer", description: "Unique identifier for the target message thread (topic)" },
        live_photo: { type: "string", description: "Live photo to send. Pass a file_id or HTTP URL" },
        caption: { type: "string", description: "Caption, 0-1024 characters" },
        parse_mode: { type: "string", description: "Mode for parsing entities (HTML, Markdown, MarkdownV2)" },
        caption_entities: { type: "array", description: "List of special entities in caption" },
        show_caption_above_media: { type: "boolean", description: "Pass True to show caption above the media" },
        disable_notification: { type: "boolean", description: "Sends the message silently" },
        protect_content: { type: "boolean", description: "Protects content from forwarding and saving" },
        allow_paid_broadcast: { type: "boolean", description: "Allow paid broadcast" },
        message_effect_id: { type: "string", description: "Unique identifier of the message effect" },
        reply_parameters: { type: "object", description: "Description of the message to reply to" },
        reply_markup: { type: "object", description: "Additional interface options" },
      },
      required: ["chat_id", "live_photo"],
    },
  },
];
